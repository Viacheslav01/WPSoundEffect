...
WaveData::WaveData(const Array<byte>^ buffer)
{
	_buffer = buffer;
	Read(_buffer);
}

void WaveData::Read(const Array<byte>^ buffer)
{
	if (!buffer)
	{
...
ChunkInfo WaveData::FindChunk(const Array<byte>^ buffer, const RiffType& chunkType) const
{
	if (buffer == nullptr)
	{
		throw ref new Platform::InvalidArgumentException("buffer is null");
